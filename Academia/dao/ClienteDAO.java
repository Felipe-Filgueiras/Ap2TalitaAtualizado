...
public class ClienteDAO {
    ...
    public Cliente recuperarUmClientes(int id_cliente) {

        try {
            String sql = "SELECT id_cliente, nome, cpf, telefone, plano FROM cliente WHERE id_cliente = ?";
    ...
    public void alterarCliente(Cliente novoCliente) {
        try {
            String sql = "UPDATE cliente SET nome = ?, cpf = ?, telefone = ?, plano = ? WHERE id_cliente = ?";
            
            try (PreparedStatement pstm = connection.prepareStatement(sql)) {
                pstm.setString(1, novoCliente.getNome());
                pstm.setString(2, novoCliente.getCpf());
                pstm.setString(3, novoCliente.getTelefone());
                pstm.setString(4, novoCliente.getPlano());
                pstm.setInt(5, novoCliente.getIdCliente());
    
                int rowsAffected = pstm.executeUpdate();
    
                if (rowsAffected == 0) {
                    throw new RuntimeException("No client found with ID: " + novoCliente.getIdCliente());
                }
            }
        } catch (SQLException e) {
            throw new RuntimeException(e);
